...
export default {
  ...
  created() {
    ...
    this.record = {},
    this.statusList = [{ "id": 0, "statusName": "在用" }, { "id": 1, "statusName": "闲置" }, { "id": 2, "statusName": "报废" }]
  ...
  methods: {
    confirm() {
      ...
      verificationApi
        .check(this.record).then((response) => {
        this.$message({
          type: 'success',
          message: '录入成功!'
        })
      ...
      this.$router.push({ path: `/verification/records` })
    }
  }
}
</script>
