    ...
    <el-form label-width="120px">
      ...
      <el-form-item label="物品图示">
        <el-upload :show-file-list="false" :on-success="handleCoverSuccess" :before-upload="beforeCoverUpload"
          ...
          action="http://localhost:8140/admin/goods/upload?module=cover">
          <img v-if="goods.cover" :src="goods.cover">
...
export default {
  ...
  created() {
    ...
    if (this.goods.id) { this.init() }
  },
  methods: {
    initKindList() {
      goodsApi.listKind().then(r => {
        this.kindList = r.data
      })
    },
    init() {
      goodsApi.getGoodsById(this.goods.id).then(response => {
